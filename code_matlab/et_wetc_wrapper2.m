...
% wrapper for the wetc C code
%   inputs are (1) path to alignment (keep short)
%              (2) base name for output files
%              (3) scoring schema (realval, entropy, integer ET)
%              (4) name of the query sequence in MSA
%

  %wetc_code = 'noah ../et_bin/wetc';
  TEMP_DIR = '../et_temp';
  ...
  % set wetc command
  cmd = sprintf('noah "../et_bin/wetc -p %s -x %s -%s -dnaet -o %s -skip_query"',...
                 aln_fp, query, scoring, outname);
  disp(cmd);
  x = system(cmd);
